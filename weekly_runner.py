...
LATITUDE  = float(os.getenv("SENSOR_LATITUDE", "40.743336"))   # Decimal degrees, -90  to  90
LONGITUDE = float(os.getenv("SENSOR_LONGITUDE", "-84.24808"))  # Decimal degrees, -180 to 180
...
def build_email_charts(
    ...
    if precip_col:
        ...
        past_precip["Period"] = "Past 7 days"
    ...
    if precip_frames:
        ...
        if not precip_data.empty:
            ...
            colors = precip_data["Period"].map({"Past 7 days": "#5B9BD5", "Forecast": "#70AD47"}).fillna("#A5A5A5")
            ax.bar(precip_data["Date"].astype(str), precip_data["Precipitation (mm)"], color=colors)
            ax.set_title("Precipitation: Past 7 Days and Forecast")
            ax.set_ylabel("mm")
            ax.tick_params(axis="x", rotation=35)
            ax.grid(axis="y", alpha=0.25)
            ax.spines[["top", "right"]].set_visible(False)
            handles = [
                plt.Rectangle((0, 0), 1, 1, color="#5B9BD5", label="Past 7 days"),
    ...
    if temp_col:
        ...
        temp_series.append(("Past 7 days min", past_temp["Date"], past_temp["min"], "#5B9BD5", "--"))
        temp_series.append(("Past 7 days max", past_temp["Date"], past_temp["max"], "#C00000", "--"))
    ...
    if temp_series:
        ...
        for label, dates, values, color, linestyle in temp_series:
            ...
            plotted = True
        if plotted:
            ax.set_title("Min and Max Temperature: Past 7 Days and Forecast")
            ax.set_ylabel("°C")
            ax.tick_params(axis="x", rotation=35)
            ax.grid(axis="y", alpha=0.25)
            ax.spines[["top", "right"]].set_visible(False)
...
def _daily_summary_html(df_summary: pd.DataFrame) -> str:
    ...
    return f"""
    <h3 style="color:#1F4E79;">📊 Past 7 Days — Daily Sensor Summary (Zentra)</h3>
...
def _charts_html(chart_paths: dict[str, str] | None) -> str:
    ...
    if "precip_chart" in chart_paths:
        charts.append("""
        <h3 style="color:#1F4E79;">Precipitation: Past 7 Days and Forecast</h3>
    ...
    if "temp_chart" in chart_paths:
        charts.append("""
        <h3 style="color:#1F4E79;">Min and Max Temperature: Past 7 Days and Forecast</h3>
...
def build_email_body(
    ...
    forecast_block = _forecast_html(forecast_rows, "7-Day Future Forecast (Open-Meteo)")
    second_week_forecast_block = _forecast_html(
        second_week_forecast_rows,
        "8-14 Day Future Forecast (Open-Meteo)",
    )
    ...
    <body style="font-family:Arial,sans-serif;color:#333;max-width:700px;margin:auto;">
      ...
      <!-- ── Past 7 days daily summary (Zentra) ── -->
      {summary_block}

      <!-- ── 7-day forecast (Open-Meteo) ── -->
      {forecast_block}

      <!-- ── 8-14 day forecast (Open-Meteo) ── -->
      {second_week_forecast_block}

      {charts_block}

      <p style="margin-top:24px;font-size:12px;color:#888;">
        The full hourly Excel file is attached to this email and also saved to
...
def main(dashboard_only: bool = False):
    ...
    try:
        ...
        send_email(subject, html_body, attachment_path=out_path, inline_images=chart_paths)
